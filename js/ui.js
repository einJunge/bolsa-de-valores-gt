...
var UI = (function () {
  ...
  function renderWatchlist() {
    ...
    if (tabsEl && !tabsEl.dataset.bound) {
      ...
      tabsEl.innerHTML = ["stock", "etf", "commodity", "forex"].map(function (c) {
  ...
  function renderReportPanel() {
    ...
    el.innerHTML =
      ...
      '<p class="muted-mono small">Descarga el informe en PDF o DOCX con la distribución de US$50,000, selección de instrumentos, justificación, riesgo, escenarios, bitácora e historial — listo para entrega universitaria.</p></div>' +
      '<span class="cyan">' + (reportPanelOpen ? "Ocultar ▲" : "Configurar ▼") + "</span></button>" +
      (reportPanelOpen ? '<div class="report-form">' +
        '<div class="report-grid">' +
        '<p class="report-note span2">La simulación inicia con <strong>US$50,000.00</strong> y la cartera académica recomendada ya está cargada: SPY, QQQ, MSFT, JPM y XAUUSD. Ambos formatos incluyen monto, porcentaje, riesgo, escenarios, bitácora y motivo de selección de cada inversión.</p>' +
        field("reportTitle", "Título del informe", "Informe de Simulación y Distribución de Inversiones", true) +
        ...
        "</div>" +
        '<div class="report-actions span2"><button id="reportSubmit" class="submit-btn amber">Descargar informe PDF</button><button id="reportDocxSubmit" class="submit-btn cyan-btn">Descargar informe DOCX</button></div>' +
    ...
    if (reportPanelOpen) {
      function buildReportMeta() {
        return {
          reportTitle: val("reportTitle") || "Informe de Simulación y Distribución de Inversiones",
      ...
      $("#reportSubmit").addEventListener("click", function () {
        ...
        try {
          InvestmentReport.generate(buildReportMeta());
          fb.textContent = "Informe PDF descargado correctamente.";
          fb.className = "feedback ok";
        } catch (err) {
          fb.textContent = "No se pudo generar el PDF: " + err.message;
          fb.className = "feedback err";
        }
      });
      $("#reportDocxSubmit").addEventListener("click", function () {
        var fb = $("#reportFeedback");
        fb.textContent = "Generando el informe DOCX…";
        fb.className = "feedback";
        InvestmentReport.generateDocx(buildReportMeta()).then(function () {
          fb.textContent = "Informe DOCX descargado correctamente.";
          fb.className = "feedback ok";
        }).catch(function (err) {
          fb.textContent = "No se pudo generar el DOCX: " + err.message;
          fb.className = "feedback err";
        });
      });
    }
  ...
  function init() {
    ...
    if (resetBtn) {
      resetBtn.addEventListener("click", function () {
        if (confirm("¿Reiniciar la simulación? Esto borra todas las posiciones, órdenes, apuestas, historial y bitácora. La cuenta quedará con US$50,000 en efectivo y sin posiciones.")) {
          Portfolio.reset().then(function () {
            if (window.PortfolioAnalytics && typeof PortfolioAnalytics.reset === "function") {
              PortfolioAnalytics.reset();
            }
            renderCurrentView();
          }).catch(function (err) {
            alert("No se pudo reiniciar la cuenta remota. No se borraron los datos para evitar inconsistencias. " + (err.message || ""));
          });
        }
      });
    }
    ...
    if (loadPlanBtn) {
      loadPlanBtn.addEventListener("click", function () {
        if (confirm("¿Cargar la cartera recomendada? Reemplazará las posiciones actuales por la distribución académica de US$50,000.")) {
          Portfolio.loadRecommendedPlan().then(function () {
            renderCurrentView();
          }).catch(function (err) {
