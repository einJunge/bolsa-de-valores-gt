...
var Portfolio = (function () {
  ...
  var saveTimer = null;
  var supabaseWriteQueue = Promise.resolve();

  function emptyState() {
    return { cash: STARTING_BALANCE, startingBalance: STARTING_BALANCE, positions: [], orders: [], bets: [], strategyLoaded: false };
  }

  function defaultState() {
    var positions = BolsaData.INVESTMENT_PLAN.map(function (plan) {
      var instrument = BolsaData.find(plan.symbol);
      var quantity = plan.allocation / instrument.basePrice;
      return {
        symbol: plan.symbol, category: instrument.category, quantity: quantity, avgPrice: instrument.basePrice,
        optionType: null, strike: null, expiry: null
      };
    });
    return { cash: 0, startingBalance: STARTING_BALANCE, positions: positions, orders: [], bets: [], strategyLoaded: true };
  }

  function normalize(parsed) {
    if (!parsed || typeof parsed.cash !== "number") return defaultState();
    /* La versión revisada de la actividad exige un capital inicial de US$50,000. */
    if (parsed.startingBalance !== STARTING_BALANCE) return defaultState();
  ...
  function saveSupabaseNow() {
    if (backendMode !== "supabase" || !supabaseUserId || !db) return Promise.resolve();
    var snapshot = JSON.parse(JSON.stringify(db));
    var userId = supabaseUserId;
    var client = Auth.getClient();
    supabaseWriteQueue = supabaseWriteQueue.catch(function () {}).then(function () {
      return client.from("user_portfolios")
        .upsert({ user_id: userId, data: snapshot, updated_at: new Date().toISOString() });
    }).then(function (res) {
      if (res.error) throw res.error;
      return res;
    }).catch(function (err) {
      console.error("[BolsaGT] error guardando en Supabase:", err.message || err);
      throw err;
    });
  ...
  function flush() {
    clearTimeout(saveTimer);
    if (backendMode === "supabase") return saveSupabaseNow();
  ...
  function reset() {
    ...
    if (backendMode === "local") {
      ...
      return saveSupabaseNow().then(function () { return db; });
    }
  }
  ...
  return {
    ...
    getSummary: getSummary, getOrders: getOrders, getOpenOrders: getOpenOrders, reset: reset, loadRecommendedPlan: loadRecommendedPlan,
