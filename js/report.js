...
var InvestmentReport = (function () {
  ...
  function reportNews() {
    if (typeof NewsEngine === "undefined" || typeof NewsEngine.getFeed !== "function") return [];
    return NewsEngine.getFeed() || [];
  }
  ...
  function findings(rng, summary) {
    ...
    var emptyPosition = { symbol: "sin posiciones", unrealizedPnlPct: 0, unrealizedPnl: 0 };
    var best = sorted[0] || emptyPosition, worst = sorted[sorted.length - 1] || emptyPosition;
  ...
  function generate(meta) {
    ...
    y = subheading(doc, "2.1 Plan académico recomendado", y);
    y = paragraph(doc, "Esta distribución es una recomendación educativa y no representa posiciones ejecutadas. La cartera solo se carga cuando el estudiante pulsa Cargar plan recomendado; después de Reiniciar, la cuenta conserva US$50,000 en efectivo y cero posiciones.", y + 2, contentWidth) + 8;
    ...
    if (y > doc.internal.pageSize.getHeight() - 160) { doc.addPage(); y = 56; }
    y = subheading(doc, "5.2 Noticias y eventos del periodo", y + 12);
    if (newsFeed.length === 0) {
      y = paragraph(doc, "No se han publicado noticias simuladas todavía. El feed se genera durante la sesión y aparecerá en el próximo informe.", y, contentWidth);
    } else {
      doc.autoTable({
        startY: y,
        margin: { left: MARGIN, right: MARGIN },
        head: [["Fecha", "Categoría", "Impacto", "Símbolos", "Titular", "Análisis"]],
        body: newsFeed.map(function (n) {
          return [new Date(n.publishedAt).toLocaleString("es-GT"), n.category || "—", n.impacted ? "Movió el mercado" : "Sin impacto notorio", (n.symbols || []).join(", "), n.headline || "—", n.body || "—"];
        }),
        styles: { font: "helvetica", fontSize: 7, textColor: INK_SOFT, cellPadding: 3, valign: "top", overflow: "linebreak" },
        headStyles: { fillColor: INK, textColor: [255, 255, 255], fontStyle: "bold" },
        alternateRowStyles: { fillColor: [244, 246, 247] }
      });
  ...
  async function generateDocx(meta) {
    ...
    children.push(docxParagraph("2.1 Plan académico recomendado", { heading: D.HeadingLevel.HEADING_2 }));
    children.push(docxParagraph("Esta distribución es una recomendación educativa y no representa posiciones ejecutadas. La cartera solo se carga cuando el estudiante pulsa Cargar plan recomendado; después de Reiniciar, la cuenta conserva US$50,000 en efectivo y cero posiciones.", { after: 160 }));
    ...
    if (orders.length) {
      children.push(docxTable(["Fecha", "Símbolo", "Lado", "Tipo", "Cantidad", "Precio", "Estado"], orders.map(function (o) {
    ...
    if (bets.length) children.push(docxTable(["Fecha", "Símbolo", "Dirección", "Duración", "Inversión", "Resultado", "P/L"], bets.map(function (b) {
    ...
    children.push(docxParagraph("5.2 Noticias y eventos del periodo", { heading: D.HeadingLevel.HEADING_2 }));
    if (newsFeed.length) children.push(docxTable(["Fecha", "Categoría", "Impacto", "Símbolos", "Titular", "Análisis"], newsFeed.map(function (n) { return [new Date(n.publishedAt).toLocaleString("es-GT"), n.category || "—", n.impacted ? "Movió el mercado" : "Sin impacto notorio", (n.symbols || []).join(", "), n.headline || "—", n.body || "—"]; })));
    else children.push(docxParagraph("No se han publicado noticias simuladas todavía."));
    ...
    if (decisions.length) children.push(docxTable(["Fecha", "Activo", "Acción", "Monto", "Expectativa", "Motivo"], decisions.map(function (item) { return [new Date(item.date).toLocaleDateString("es-GT"), item.symbol, item.action, money(item.amount), item.expected || "—", item.reason]; })));
